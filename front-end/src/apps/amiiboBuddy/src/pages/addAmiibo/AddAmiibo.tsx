...
export const AddAmiibo: React.FC<{}> = () => {
  ...
  const uploadImagesToSupabaseBuckets = async (packId: string) => {
    const data = await Promise.all<{
      ...
      fileList.map((file) => uploadImageToSupabase(file.originFileObj, packId)),
    )
      .then((data) => {
  ...
  const onFinish = (values: any) => {
    ...
    const packData = finishedFormValues.amiibos.map((amiibo: TAmiiboCard) => ({
      user_id: userId,
      amiibo_id: amiibo.id,
      location: locationName,
      price: values.price,
  ...
  if (!isLoading) {
    return (
      <>
        ...
        >
          {numberOfAmiibos > 1 ?
            <Form.Item label="Pack Name" name="packName" labelCol={{ span: 6 }}>
              <Input />
            </Form.Item>
          : amiibosArr[0] ?
            <p
          ...
          >
            ...
            >
              <Select
                ...
                onChange={(value) => handleNumberOfAmiiboChange(value)}
              >
            ...
            <Space className="currency-price">
              ...
              >
                <InputNumber<string>
                  ...
                  stringMode
                  defaultValue="0.00"
                  parser={(value) => parseFloat(value!).toFixed(2).toString()}
                  prefix={<>{currency}</>}
                />
              </Form.Item>
            ...
            >
              {options && (
                <AutoComplete
                  ...
                  onSelect={(value) => handleAmiiboSelect(value, index)}
          ...
          <div className="amiibo-image-banner">
            {amiibosArr.map(
              (amiibo, index) =>
                amiibo && (
                  <div className="amiibo-image-card" key={index}>
                    <h4
