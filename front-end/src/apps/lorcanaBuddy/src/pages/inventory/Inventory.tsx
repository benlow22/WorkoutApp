...
export const Inventory = () => {
	...
	const [usersUpdatedCardStatuses, setUsersUpdatedCardStatuses] = useState<TCardStatus>({});
	// const [usersCardStatuses, setUsersCardStatuses] = useState<TCardStatus>({});
	const [messageApi, contextHolder] = message.useMessage();

	const [getAllLorcanaCardsResponse, getAllLorcanaCardsLoading, getAllLorcanaCardsError, getAllLorcanaCardsRequest] = useRequest(getAllLorcanaCardsAPI);
	...
	const getUsersCardStatuses = async () => {
		let { data, error } = await supabase.from("lorb_cards_statuses").select("cards_statuses").single();
		if (error) {
	...
	const handleFloatSave = async () => {
		if (!auth) {
			...
			setIsUploading(true);
			const { data, error } = await supabase.from("lorb_cards_statuses").update({ cards_statuses: usersUpdatedCardStatuses }).eq("user_id", userId).select();
			if (data) {
				...
				<div className="card-grid">
					{lorcanaAllCards.map((card, index) => (
						<LorcanaCard
							...
							key={index}
							card={card}
							setUsersUpdatedCardStatuses={setUsersUpdatedCardStatuses}
