...
import {
  TNewCard,
  TNewCardAndUserData,
} from "../../pages/newInventory/NewInventory";
...
export const NewCardFilterMenu = ({
  ...
  const [cardRarityFilters, setCardRarityFilters] = useState<
    CheckboxValueType[]
  >([]);
  ...
  const valuesToSearchParams = (
    searchParams: URLSearchParams,
    filter: CheckboxValueType[],
    filterName: string,
  ) => {
    if (filter) {
  ...
  useEffect(() => {
    const getDistinctWavesFromDB = async () => {
      const { data, error } = await supabase
        .rpc("get_distinct_values_from_new_cards")
        .select("*");
      ...
      data.map((row: { wave: string }, index) =>
        typedData.push({ label: row.wave, value: index + 1 }),
      );
  ...
  const cardInkFilterFn = (card: TNewCard) =>
    cardInkFilters.includes(card.color);
  // const cardInkFilterFn = (card: TNewCard) => {
  ...
  const cardInkFilterOptions = [
    {
      label: (
        <div
          className={
            ((
              cardInkFilters.toString() === "Amber" ||
              cardInkFilters.includes("Amber")
            ) ?
              "active "
            : "not-active ") + "amber-ink"
          }
        >
          <img
            src="/amber.svg"
            style={{ width: "50px", padding: "0px", position: "relative" }}
          />
        </div>
      ),
      value: "Amber",
    },
    {
      label: (
        <div
          className={
            ((
              cardInkFilters.toString() === "Amethyst" ||
              cardInkFilters.includes("Amethyst")
            ) ?
              "active "
            : "not-active ") + "amethyst-ink"
          }
        >
          <img
            src="/amethyst.svg"
    ...
    },
    {
      label: (
        <div
          className={
            ((
              cardInkFilters.toString() === "Emerald" ||
              cardInkFilters.includes("Emerald")
            ) ?
              "active "
            : "not-active ") + "emerald-ink"
          }
        >
          <img
            src="/emerald.svg"
    ...
    },
    {
      label: (
        <div
          className={
            ((
              cardInkFilters.toString() === "Ruby" ||
              cardInkFilters.includes("Ruby")
            ) ?
              "active "
            : "not-active ") + "ruby-ink"
          }
    ...
    },
    {
      label: (
        <div
          className={
            ((
              cardInkFilters.toString() === "Sapphire" ||
              cardInkFilters.includes("Sapphire")
            ) ?
              "active "
            : "not-active ") + "sapphire-ink"
          }
        >
          <img
            src="/sapphire.svg"
    ...
    },
    {
      label: (
        <div
          className={
            ((
              cardInkFilters.toString() === "Steel" ||
              cardInkFilters.includes("Steel")
            ) ?
              "active "
            : "not-active ") + "steel-ink"
          }
        >
          <img
            src="/steel.svg"
